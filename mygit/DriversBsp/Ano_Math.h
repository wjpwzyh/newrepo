...
#define my_pow_2_curve(in, a, max) \
  (((1.0f - (a)) + (a)*LIMIT(ABS((in) / (max)), 0, 1)) * in)

//#define RECTANGLE2CIRCLE_FACTOR(x,y,l)
//(safe_div(_MAX(ABS((x)),ABS((y))),(l),0)) #define RECTANGLE2CIRCLE_FACTOR(r,l)
//(safe_div((r),(l),0))

#define To_180_degrees range_to_180deg
#define range_to_180deg(a) \
