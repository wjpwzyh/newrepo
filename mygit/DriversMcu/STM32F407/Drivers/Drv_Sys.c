...
void SysTick_Handler(void) { SysRunTimeMs++; }
uint32_t GetSysRunTimeMs(void) { return SysRunTimeMs; }
uint32_t GetSysRunTimeUs(void) {
  ...
  do {
    ms = SysRunTimeMs;
    value =
        ms * TICK_US + (SysTick->LOAD - SysTick->VAL) * TICK_US / SysTick->LOAD;
