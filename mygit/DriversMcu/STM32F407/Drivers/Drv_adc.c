...
static void initAdcDMA(void) {
  ...
  DMA_InitStructure.DMA_MemoryInc =
      DMA_MemoryInc_Enable;  // DMA_MemoryInc_Disable;/*地址不增加*/
...
void DrvAdcInit(void) {
  ...
  ADC_CommonInitStructure.ADC_Prescaler =
      ADC_Prescaler_Div4; /*APB2的4分频 即84/4=21M*/
  ADC_CommonInitStructure.ADC_TwoSamplingDelay =
      ADC_TwoSamplingDelay_5Cycles; /*两个采样阶段的延时5个时钟*/
  ...
  ADC_RegularChannelConfig(
      ...
      ADC_SampleTime_480Cycles); /*设置规则通道3 二个序列 采样时间 */
